...
<body class="bg-gradient-to-br from-orange-50 to-amber-100 min-h-screen flex items-center justify-center p-4 sm:p-8 font-sans">
  ...
  <div class="w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-fade-in-up">
    ...
    <div class="md:w-3/5 p-8 sm:p-12 relative">
        ...
        <form action="../controllers/Controller-cadastro.php" method="POST" enctype="multipart/form-data" class="space-y-6">
            
            <!-- Campo de foto e Preview -->
            <div class="group relative">
                <div id="preview-container" class="w-full h-48 bg-amber-50 rounded-2xl border-2 border-dashed border-amber-200 flex flex-col items-center justify-center text-amber-400 transition-all duration-300 group-hover:border-amber-400 group-hover:bg-amber-50/80 overflow-hidden relative cursor-pointer">
                    
                    <!-- Imagem Preview -->
                    <img id="preview" class="absolute inset-0 w-full h-full object-cover hidden">
                    
                    <!-- Placeholder e Input -->
                    <label for="foto_produto" class="absolute inset-0 flex flex-col items-center justify-center cursor-pointer z-10">
                        <div id="upload-icon" class="bg-white p-3 rounded-full shadow-sm mb-2 group-hover:scale-110 transition-transform">
                            <i class="fas fa-camera text-2xl text-amber-500"></i>
                        </div>
                        <span id="upload-text" class="text-sm font-bold text-amber-600">Clique para adicionar foto</span>
                        <input type="file" id="foto_produto" name="foto_produto" accept="image/*" class="hidden">
                    </label>
                </div>
            </div>
            ...
            <div class="space-y-5">
                ...
                <!-- Nome -->
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1 ml-1">Nome do Produto</label>
                    <div class="relative">
                ...
                <div class="grid grid-cols-2 gap-5">
                    <!-- Preço -->
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1 ml-1">Preço (R$)</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    ...
                    <!-- Quantidade -->
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1 ml-1">Estoque</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            ...
            <button type="submit" name="btn-cadastrar" class="w-full bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-bold text-lg py-4 rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-3">
